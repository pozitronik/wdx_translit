...
type
  ...
  String04 = string[4];                          { AnsiString with max. 4 symbols }
  String30 = string[30];                        { AnsiString with max. 30 symbols }
  ...
  TID3v1 = class(TObject)
    ...
    public
      ...
      property Genre: AnsiString read FGetGenre;                     { Genre name }
  end;
...
function ReadTag(const FileName: WideString; var TagData: TagRecord): Boolean;
var
  SourceFile: file;
begin
  try
...
function RemoveTag(const FileName: WideString): Boolean;
var
  SourceFile: file;
begin
  try
...
function SaveTag(const FileName: WideString; TagData: TagRecord): Boolean;
var
  SourceFile: file;
begin
  try
...
begin
  Result := '';
  { Return an empty AnsiString if the current GenreID is not valid }
...
function TID3v1.RemoveFromFile(const FileName: WideString): Boolean;
var
  TagData: TagRecord;
begin
  { Find tag }
...
begin
  ...
  begin
    ...
    TagData.Comment[30] := AnsiChar(Chr(FTrack));
  end;
