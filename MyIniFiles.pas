...
{ Corrections and bugfixes: D1P 2005                                          }
{ *************************************************************************** }
...
type
  ...
  public
    ...
    function ReadString(const Section, Ident, Default: string; CaseSensivity:boolean): string; virtual; abstract;
  ...
  public
    ...
    function ReadString(const Section, Ident, Default: string; CaseSensivity:boolean): string; override;
...
var
  IntStr: string;
begin
  IntStr := ReadString(Section, Ident, '',false);
...
var
  DateStr: string;
begin
  DateStr := ReadString(Section, Name, '',false);
  Result := Default;
  if DateStr <> '' then
  try
...
var
  FloatStr: string;
begin
  FloatStr := ReadString(Section, Name, '',false);
  Result := Default;
  if FloatStr <> '' then
  try
...
var
  TimeStr: string;
begin
  TimeStr := ReadString(Section, Name, '',false);
  Result := Default;
  if TimeStr <> '' then
  try
...
var
  ...
  Pos: Integer;
begin
  Text := ReadString(Section, Name, '',False);
  if Text <> '' then
  begin
...
function TMemIniFile.ReadString(const Section, Ident, Default: string; CaseSensivity:boolean): string;
var
  I: Integer;
  Strings: TStrings;
begin
...
function TIniFile.ReadString(const Section, Ident, Default: string; CaseSensivity:boolean): string;
var
 Buffer: array[0..2047] of Char;
 S:string;
 f:textFile;
begin
if CaseSensivity then
 begin
 result:=default;
 try
  ...
  repeat
  readln (f,s);
  until (EOF (f)) or (s='['+Section+']');
  if not EOF (F) then
   begin
   repeat
   readln (f,s);
   until (copy (s,1,pos ('=',s)-1)=ident) or EOF (F);
   if not ((EOF (f)) and ((copy (s,1,pos ('=',s)-1)<>ident))) then if s<>'' then result:=copy (s,pos ('=',s)+1,length (s)-pos ('=',s)+1);
   end;
  CloseFile (f);
 except
 on e: exception do
  begin
  MessageBox (GetDesktopWindow,'Ошибка в модуле MyIniFiles, пожалуйста, сообщите автору.','БУ!', mb_iconerror);
  end;
 end;
end else SetString(Result, Buffer, GetPrivateProfileString(PChar(Section), PChar(Ident), PChar(Default), Buffer, SizeOf(Buffer), PChar(FFileName)));
  ...
  try
    ...
    try
      ...
      for I := 0 to KeyList.Count - 1 do
        Strings.Add(KeyList[I] + '=' + ReadString(Section, KeyList[I], '',False))
    finally
      Strings.EndUpdate;
    end;
  finally
